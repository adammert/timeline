    ...
    <div class="input-panel">
      <div class="header-controls">
        <div class="header-buttons">
          <button class="templates-toggle" id="templatesToggle" title="Templates">📄</button>
          <button class="stats-toggle" id="statsToggle" title="Statistiken">📊</button>
...
> Super Fortschritt!"></textarea>
      ...
      <button id="visualizeBtn">Präsentation öffnen</button>
      <div class="keyboard-hint">
        Strg+Enter: Präsentation | Strg+S: Speichern | Strg+F: Suche | Strg+V: Screenshot einfügen
      </div>
    ...
    <div class="modal-content">
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-label">Zukünftige Ereignisse</div>
        </div>
        <div class="stat-card">
  ...
  <div id="debugModal" class="modal">
    <div class="modal-content">
      ...
      <button id="testDropZone"
        ...
        📎 Bild hierher ziehen zum Testen
      </button>
    </div>
  </div>
  ...
  <script>
    ...
    async function runDebugTests() {
      ...
      function log(msg, isError = false) {
        ...
        html += `<div style="color: ${color}; margin: 5px 0;">${isError ? '❌' : '✅'} ${msg}</div>`;
      ...
      try {
        ...
        if (!window.indexedDB) {
          log('IndexedDB wird nicht unterstützt!', true);
          return;
        }
        log('IndexedDB wird unterstützt');
        ...
        try {
          ...
          if (retrieved) {
            ...
            log('Test-Bild erfolgreich gelöscht');
          } else {
        ...
        if ('FileReader' in window) {
          log('FileReader API verfügbar');
        } else {
          log('FileReader API nicht verfügbar', true);
        }

        // Test 10: Clipboard API
        if (navigator.clipboard) {
          log('Clipboard API verfügbar');
        } else {
          log('Clipboard API eingeschränkt (benötigt HTTPS)', true);
        }
            ...
            for (let i = 0; i < e.dataTransfer.files.length; i++) {
              ...
              if (file.type.startsWith('image/')) {
                try {
                  ...
                  if (filename) {
                    html += `<span style="color: green;">✅ Erfolgreich gespeichert als: ${filename}</span><br>`;
                  } else {
                    html += '<span style="color: red;">❌ Speichern fehlgeschlagen</span><br>';
                  }
                } catch (err) {
                  html += `<span style="color: red;">❌ Fehler: ${err.message}</span><br>`;
                }
              }
            }
          } else {
