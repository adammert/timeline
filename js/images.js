/**
 * Image Management Module - Debug Version
...
TimelineApp.Images = {
  ...
  async init() {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onblocked = () => {
        console.warn(
          "IndexedDB blocked - close other tabs using this database"
        );
        alert(
  ...
  async handleImageFile(file) {
    ...
    if (file.size > maxSize) {
      alert(
        `Das Bild "${file.name}" ist zu groß (${Math.round(
          file.size / 1024 / 1024
        )}MB).\n` + `Maximum: 10MB`
      );
      return null;
  ...
  insertImageMarkdown(textarea, filename, altText = "") {
    ...
    const newPosition =
      start + (needsNewlineBefore ? 1 : 0) + markdownImage.length;
  ...
  async replaceImageReferences(container) {
    ...
    for (const img of images) {
      ...
      try {
        ...
        img.alt = `[Fehler beim Laden: ${filename}]`;
        img.style.color = "red";
        img.style.display = "block";
      }
    }
